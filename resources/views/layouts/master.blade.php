                ...
                x-show="menu === true" x-on:click.away="menu = false" class="absolute inset-x-0 top-0 z-50 p-2 transition origin-top-right transform lg:hidden">
                  <div class="bg-white divide-y-2 rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 divide-gray-50">
                      ...
                      <div class="mt-6">
                        <nav class="grid gap-y-8">
                          ...
                          x-transition:enter="transition-all ease-out duration-200"
                            ...
                            x-transition:leave-start="opacity-100 translate-y-0"
                            x-transition:leave-end="opacity-0 translate-y-1"
                            -cloak x-show="about === true" x-on:click.away="about = false" class="grid px-5 gap-y-8">
                            @foreach (Menu::where('type', 'about')->orderBy('name_en', 'asc')->where('active', true)->get() as $body)
                    ...
                    @foreach (Menu::where('type', '!=','about')->where('parent',null)->orderBy('name_en', 'asc')->where('active', true)->get() as $body)
                        ...
                        @else
                        <div x-data="{open: false}" class="relative">
                            <button x-on:click="open = !open" type="button" class="flex items-center justify-between p-3 -m-3 rounded-md hover:bg-gray-50" aria-expanded="false">
                              <span class="text-base font-medium text-gray-900">
                                @if(App::isLocale('en'))
                                    {!! $body->name_en !!}
                                @elseif(App::isLocale('id'))
                      ...
                      @endforeach
                          ...
                          @foreach (Menu::where('type', '!=','about')->where('parent',null)->orderBy('name_en', 'asc')->where('active', true)->get() as $body)
                            ...
                            x-cloak x-show="investor === true" x-on:click.away="investor = false" class="grid px-5 gap-y-8">
                                ...
                                @foreach($body->child as $m)
                                    @if($m->type == 'page')
                                        <a href="{{ url($body->slug .'/'. $m->slug) }}" class="flex items-center p-3 -m-3 rounded-md hover:bg-gray-50">
                                            <p class="text-base font-medium text-gray-900">
                                                @if(App::isLocale('en'))
                                                    {!! $m->name_en !!}
                                                @elseif(App::isLocale('id'))
                                    ...
                                    @else
                                        <div x-data="{open:false}" x-on:click="open = !open" class="relative flex items-center justify-between p-3 -m-3 rounded-md cursor-pointer hover:bg-gray-50">
                                            <p class="text-base font-medium text-gray-900">
                                                @if(App::isLocale('en'))
                                                    {!! $m->name_en !!}
                                                @elseif(App::isLocale('id'))
                                            ...
                                            x-cloak x-show="open === true" @click.away="open = false" class="grid px-5 gap-y-8">
                                                @foreach($m->child as $sm)
                                                    <a href="{{ url($body->slug .'/'. $m->slug .'/'. $sm->slug) }}" class="flex items-center p-3 -m-3 rounded-md hover:bg-gray-50">
                                                        <p class="text-base font-medium text-gray-900">
                                                            @if(App::isLocale('en'))
                                                                {!! $sm->name_en !!}
                                                            @elseif(App::isLocale('id'))
